...
function initMenuTree(islink) {
    ...
    $("#sidebar-nav .list-box .list-group").each(function (i) {
        ...
        initMenu();
        //为H2添加事件
        $(this).children("h2").each(function (id) {
            $(this).click(function () {
                ...
                $("#nav li").removeClass("selected").eq(id).addClass("selected");
            });
        });
...
//
function initMenu() {
    $("#sidebar-nav .list-box .list-group>h2").each(function (i) {
        var navHtml = $('<li><i class="icon-' + i + '"></i><span>' + $(this).text() + '</span></li>').appendTo($("#nav"));
        //默认选中第一项
        if (i == 0) {
            $(this).show();
        ...
        navHtml.click(function () {
            var $ul = $("#sidebar-nav .list-box .list-group>ul");
            if ($ul.eq(i).is(":hidden")) {
...
function linkMenuTree(islink, navid) {
    ...
    if (cookieObj.length > 0) {
        ...
        var curNum = $("#sidebar-nav .list-box .list-group h2").index(cookieObj.parents("ul").parents("ul").prev());
        if (curNum < 0) {
            curNum = $("#sidebar-nav .list-box .list-group h2").index(cookieObj.parents("ul").prev());
        }
        $("#nav li").eq(curNum).addClass("selected");
    } else if (argument == 2) {
        //删除所有的选中样式
        $("#sidebar-nav .list-box .list-group ul li a").removeClass("selected");
        //保存到cookie
...
function initPopMenuTree() {
    var divWidth = $("#pop-menu .list-box .list-group").length * $("#pop-menu .list-box .list-group").outerWidth() + 50;
